...
def countApplesAndOranges(s, t, a, b, apples, oranges):
    applesPositions = map(lambda apple: apple + a, apples)
    orangesPositions = map(lambda orange: orange + b, oranges)
    
    applesInHouse = 0
    for apple in applesPositions:
        if s <= apple and apple <= t:
            applesInHouse += 1

    orangesInHouse = 0
    for orange in orangesPositions:
