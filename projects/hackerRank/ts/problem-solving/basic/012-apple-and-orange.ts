...
function countApplesAndOranges(
  ...
  const applesLocations = apples.map((apple) => apple + a);
  const orangesLocations = oranges.map((orange) => orange + b);

  let appleCount = 0,
    orangeCount = 0;

  for (let apple of applesLocations) {
    if (s <= apple && apple <= t) appleCount++;
  }
  for (let orange of orangesLocations) {
    if (s <= orange && orange <= t) orangeCount++;
  }
