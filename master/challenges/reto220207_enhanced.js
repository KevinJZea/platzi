...
const isOdious = (num) => {
  ...
  const reducer = (sum, binaryDigit) => (binaryDigit === "1" ? sum + 1 : sum);

  const amountOfOnes = decimalToBinary(num).split("").reduce(reducer, 0);

  const amountOfOnesIsOdd = amountOfOnes % 2 === 1 ? true : false;
