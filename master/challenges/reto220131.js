...
const localTime = (userName, usersData = [], hourFormat = 24) => {
  if (usersData.length === 0) return `The users' data is empty.`;

  const timezone = usersData.find((user) => user.name === userName)?.timezone;
  if (!timezone) return `${userName} was not found.`;
...
humans.forEach(({ name }) =>
  console.log(localTime(name, humans), "---", localTime(name, humans, 12))
);
