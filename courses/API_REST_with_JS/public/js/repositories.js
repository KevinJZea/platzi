class ProductRepository {
  ...
  async getProduct(id) {
    ...
    if (
      !response.ok &&
      !response.headers.get('content-type') != 'applicacion/json'
    ) {
      throw new Error('No fue posible encontrar los productos');
    }
    const responseData = await response.json();
  ...
  async createProduct(product) {
    ...
    if (!response.ok) {
      throw new Error(
        `No se pudo crear el producto, con status code ${response.status}`
      );
    }
  ...
  async editProduct(productId, productData) {
    ...
    if (!response.ok) {
      throw new Error(
        `No se puede actualizar el producto ${productId}: ${response.status}`
      );
    }
